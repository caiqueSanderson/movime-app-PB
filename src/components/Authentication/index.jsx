...
export default function Authentication({ setAuthenticated }) {
  ...
  useEffect(() => {
    async function fetchAuthenticationLink() {
      try {
        const { requestToken, authenticationLink } =
          await authenticateAndGetAccountData();
  ...
  const completeAuthenticationSessionAndAccountId = async () => {
    setLoading(true);
    try {
      const { sessionId, accountId } = await completeAuthentication(
        requestToken
      );
    ...
    <div className={styles.container}>
      ...
      {loading ? (
        ...
        <div className={styles.authenticationPrompt}>
          ...
          <button
            onClick={completeAuthenticationSessionAndAccountId}
            className={styles.authenticationCompleteButton}
          >
